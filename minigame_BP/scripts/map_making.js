...
const directoryForm = new ActionFormData()
...
const mapMaker = new ModalFormData()
    .title("Map Maker")
    ...
    .slider('Number of Players', 2, 20)
    .submitButton("Submit");

const mapErase = new ModalFormData() //to do next create function that erases maps consider making a cache in the map class for easier access to these maps later
    .title('Map Eraser')
...
const tutorialform = new MessageFormData()
...
const credits = new MessageFormData()
    .title('Credits')
...
world.afterEvents.itemUse((event) => {
    const player = event.source
    const item = event.item.typeId;
    if (item = "b_minigames:map_maker") {
        directoryForm.show(player).then(response => {
           ...
           switch(selection) {
            case 0: mapMaker.show(player).then(response => {
                ...
                const numOfPlayers = response.formValues[5]
                const dataString = `data: {spawns: [${spawns}], barriers: [${barriers}], chests: [${chests}], dimension: ${dimension}, numOfPlayers: ${numOfPlayers}`
                const createdMap = new Map(name, dataString);
                createdMap.save()
            ...
            case 1: mapErase.show(player).then(response => {
                ...
                Map.allMaps.splice(chosenMapInArray, 1)
                world.setDynamicProperty(chosenMap)
            }); break;
